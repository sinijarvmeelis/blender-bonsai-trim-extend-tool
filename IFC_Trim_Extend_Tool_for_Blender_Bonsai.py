...
class IFC_OT_trim_extend(bpy.types.Operator):
    ...
    bl_description = (
        ...
        "Esc or click empty space to finish."
    )
    ...
    def invoke(self, context, event):
        ...
        if self.pick_destination:
            self.report({'INFO'}, "Click Endpoint, Midpoint or Nearest as the destination (empty click or Esc to cancel)")
        else:
            self.report({'INFO'}, "Click an IFC extrusion near the end to trim/extend (empty click or Esc to finish)")
    ...
    def modal(self, context, event):
        ...
        if self.state == 'DEST':
            ...
            if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
                if not self._event_in_view3d_window(context, event):
                    return {'PASS_THROUGH'}
                ...
                self.state = 'ELEMENT'
                self._preview = None
                self.report({'INFO'}, "Now click the element near the end to trim/extend (empty click or Esc to finish)")
                return {'RUNNING_MODAL'}
        ...
        if self.state == 'ELEMENT':
            ...
            if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
                ...
                try:
                    ...
                    self.report({'INFO'}, "Trim/Extend applied. Click another element, Ctrl+Z to undo, empty click or Esc to finish.")
                    self.target_obj = None
                    self._extrude = None
                    self._preview = None
                    return {'RUNNING_MODAL'}
    ...
    def _event_in_view3d_window(self, context, event):
        """True when the mouse is inside the 3D viewport WINDOW region (not the N-panel)."""
    ...
    def _force_viewport_redraw(self, context):
        """Redraw every 3D view so GPU overlay (snap markers, extension lines) vanishes now.

        Removing a POST_PIXEL handler does not itself swap the framebuffer. Without an
        explicit redraw the last overlay stays on screen until the user pans/zooms.
        """
        screens = []
        ...
        if wm is not None:
            for window in wm.windows:
                ...
                if win_screen is not None and win_screen not in screens:
                    screens.append(win_screen)
    ...
    def exit_tool(self, context):
        """Esc / empty-space click: drop overlay, force a viewport refresh, leave the modal."""
        self.finish(context)
    ...
    def _refresh_after_history(self, context):
        ...
        if self.pick_destination:
            try:
                ...
                self._snap_candidates = []
        self._force_viewport_redraw(context)
    ...
    def _draw_callback_px(self):
        ...
        if self.state == 'DEST':
            hint = "Click Endpoint / Midpoint / Nearest  ·  empty click or Esc cancel"
        else:
            if pv is not None and pv["valid"]:
                ...
                hint = (
                    ...
                    "  ·  LMB apply  ·  Ctrl+Z undo  ·  empty click or Esc finish"
                )
            else:
                hint = "Hover over an IFC extrusion to preview  ·  LMB apply  ·  Ctrl+Z undo  ·  empty click or Esc finish"
        draw_hint_px(region, hint)

        gpu.state.blend_set('NONE')

    def finish(self, context):
        # Clear drawable state first so a last POST_PIXEL callback paints nothing.
        # Removing the handler does not swap the framebuffer by itself; force a
        # redraw or the last snap marker / extension lines stay until the user pans.
...
class IFC_PT_trim_extend_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.label(text="Esc or click empty space to finish")
